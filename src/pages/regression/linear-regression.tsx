...
export default function LinearRegressionPage() {
  ...
  const fetchEvaluations = async () => {
    try {
      const response = await fetch('http://localhost:8000/ml/evaluations/linear_regression');
      if (response.ok) {
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/ml/predict-file/linear_regression', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="bg-background flex w-full flex-col">
      <SiteHeader heading="Linear Regression" />
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">
                Train and evaluate linear regression model for concentration prediction
              </p>
            </div>
