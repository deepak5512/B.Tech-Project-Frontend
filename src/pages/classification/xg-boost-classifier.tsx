...
export default function XGBoostClassifierPage() {
  ...
  const fetchEvaluations = async () => {
    try {
      const response = await fetch('http://localhost:8000/ml/evaluations/xgboost_classifier');
      if (response.ok) {
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/ml/predict-file/xgboost_classifier', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="bg-background flex w-full flex-col">
      <SiteHeader heading="XGBoost Classifier" />
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">
                Train and evaluate XGBoost model for gas type classification
              </p>
            </div>
