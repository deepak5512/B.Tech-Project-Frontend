...
export default function RandomForestClassifierPage() {
  ...
  const fetchEvaluations = async () => {
    try {
      const response = await fetch('http://localhost:8000/ml/evaluations/random_forest_classifier');
      if (response.ok) {
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/ml/predict-file/random_forest_classifier', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="bg-background flex w-full flex-col">
      <SiteHeader heading="Random Forest Classifier" />
      <main className="flex flex-col max-w-5xl w-full mx-auto">
        <div className="flex w-full flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold">Random Forest Classifier</h2>
              <p className="text-muted-foreground">
                Train and evaluate random forest model for gas type classification
              </p>
            </div>
